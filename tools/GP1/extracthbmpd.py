...
class XBmpRes():
    def __init__(self, s, i):
        MagicNo,Format,FrameWidth,FrameHeight,FrameDelay,NoOfFrames,Frames,Pixel =struct.unpack_from('<IiiiiiII', s, i)
        ...
        if(validateAddress(Frames) and validateAddress(Pixel)):
            ...
            self.Frames = Frames
            self.Pixel =Pixel
            #print("XBmpRes Type:%x Size:%dx%d Delay:%dms NoOfFrames:%d FramePtr:%x PixelPtr:%x" % (bmp.Format,bmp.FrameWidth,bmp.FrameHeight,bmp.FrameDelay,bmp.NoOfFrames,bmp.Frames,bmp.Pixel))
        else:
            return None

...
def accept_file(li, filename):
    ...
    size = li.size()
    b = li.read(size)    
    daddr = b.find("PIQ01_01")
    
    if (b.find("gopro") != -1) and (daddr != -1): 
        retval = "Updated GP1 Read Loader of size %d Data offset at:%x" % (size, daddr)
...
def load_file(li, neflags, format):
    ...
    li.seek(0)    
    buf = li.read(size)
    print("type of buf",type(buf))
    daddr = buf.find("PIQ01_01")
    
    if ( daddr == -1 ):
        print "Did not find PIQ01_01, leaving"
        return -1
    
    print ("found data section starting at %x" % (daddr))

    print "Assuming RTOS"
    #end = start + size
    end = END_ADDR
    DATA_ADDR = start + daddr
    ...
    idaapi.add_segm_ex(seg, ".text", "CODE", idaapi.ADDSEG_OR_DIE)
    ...
    segdata.end_ea   = end
    segdata.bitness  = 1 # 32-bit
    idaapi.add_segm_ex(segdata, ".data", "DATA", idaapi.ADDSEG_OR_DIE)
